...
public interface UserDAO extends DAO<User> {
    ...
    @SqlUpdate("INSERT INTO user (userName, password, fullName, address, gender, email, phoneNumbers, role)" +
            "VALUES (:username, :password, :fullName, :address, :gender, :email, :phoneNumbers, :role)")
    @GetGeneratedKeys("id")
    ...
    @SqlUpdate("UPDATE user SET username = :username, password = :password, fullName = :fullName, " +
            "email = :email, phoneNumbers = :phoneNumbers, gender = :gender, address = :address, role = :role " +
            "WHERE id = :id")
    ...
    @SqlUpdate("UPDATE user SET accPoint = :quantity  WHERE id = :userId ")
    void addAccumulatedPoint(@Bind("quantity") int quantity, @Bind("userId") long userId);
}
